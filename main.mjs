...
async function fetchUserItems(browser, handle) {
  const ctx = await browser.newContext({
    // 🔥 disguise as a normal Chrome user
  ...
  // add cookies like a real browser
...
(async () => {
  ...
  let out = [`📢 TikTok Report (last ${LOOKBACK_HOURS}H, min ${MIN_VIEWS} views)`];
  viral.forEach((v,i)=>{
    out.push(`${i+1}. @${v.author} — ${n(v.stats.playCount)} views`);
    out.push(`${v.url}`);
    out.push(`👍 ${n(v.stats.diggCount)} 💬 ${n(v.stats.commentCount)} • posted ${ago(now, v.createTime)}`);
    out.push("");
  });
